...
class MainApp(MDApp):
    ...
    def fetch_data(self,*args):
        ...
        if fetchVarName not in self.screenList:
            ...
            self.summary_container = self.varNameDict[fetchVarName].ids.summary_container
            self.chart_container = self.varNameDict[fetchVarName].ids.chart_container
            self.table_header = self.varNameDict[fetchVarName].ids.table_header
            self.table_container = self.varNameDict[fetchVarName].ids.table_container
            print('yfinance price history call [START]')
            self.logger0 = logging.getLogger('yfinance')
            pool = concurrent.futures.ThreadPoolExecutor(max_workers=3)
            pool.submit(self.fetch_data)
            datadf=callYfinance(fetchRic,fetchStart,fetchEnd)
            print('yfinance price history call [FINISHED]')
            if len(datadf)>0:
                ...
                #start of page summary
                print('parsing Yahoo instr. desc. [START]')
                instrNameScrape=getInstrumentName(fetchRic)
                print('parsing Yahoo instr. desc. [FINISED]')
                summary_container.add_widget(MDLabel(markup=True, text='[b]'+instrNameScrape+'[/b]',size_hint_y=None, height='15dp'))
                summary_container.add_widget(MDLabel(markup=True, text='   (queried from '+fetchStart+' to '+fetchEnd+')',size_hint_y=None, height='15dp'))
                summary_container.add_widget(MDLabel(markup=True, text='Daily performance stats:',size_hint_y=None, height='15dp'))
